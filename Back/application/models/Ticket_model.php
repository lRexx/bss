...
class Ticket_model extends CI_Model
{
	...
	public function add2($ticket)
	{
		...
		$this->db->insert('tb_tickets_2', array(
			...
			'description_internal' => @$ticket['description_internal'],
	...
	public function changueStatus($ticket)
	{
		...
		} else if ($ticket['idTypeTicketKf'] == "2") {
			$this->db->set(
				array(
					'idStatusTicketKf' => @$ticket['idNewStatusKf']
				)
		...
		if ($this->db->affected_rows() === 1) {
			...
			if ($ticket['idTypeTicketKf'] == "2" && count(@$ticket['keys']) > 0) {
				foreach ($ticket['keys'] as $key) {
					...
					$keyLogObj['newIdUserKf'] = null;
					$keyLogObj['oldIdCategoryKf'] = $key['keychain']['idCategoryKf'];
					$keyLogObj['newIdCategoryKf'] = null;
					$keyLogObj['idTicketKf'] = $ticket['idTicket'];
					$keyLogObj['idUserActionKf'] = $ticket['idUserActionKf'];
					$rsKeyEvent = $this->Llavero_model->logEvent($keyLogObj);
					if (!$rsKeyEvent) {
		...
		if (@$data['idProfileKf'] == '1' && (@$data['idClientAdminFk'] != '' || @$data['idClientCompaniFk'] != '')) {
			...
			if (@$data['idStatusTicketKf'] != '') {
				if (@$data['idStatusTicketKf'] == '10') {
					$where = "isCancelRequested = 1 AND idStatusTicketKf = " . @$data['idStatusTicketKf'];
					$this->db->where($where);
				} else if (((@$data['idDeviceTypeKf'] != '' && @$data['idDeviceTypeKf'] != '2') || (@$data['idDeviceTypeKf'] == null)) && @$data['idStatusTicketKf'] == '8' && @$data['idMgmtMethodKf'] == null && @$data['whereKeysAreEnable'] == null && @$data['isKeysEnable'] == null) {
	...
	public function buscar_relaciones_ticket($todo)
	{
		...
		foreach ($rs_tickets['tickets'] as $key => $ticket) {
			...
			if (!empty($initialDelivery)) {
				...
				$building['isInitialDeliveryActive'] 	= !$expiration_state ? true : false;
